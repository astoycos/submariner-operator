...
func Ensure(reporter eventreporter.Reporter, config *rest.Config, operatorNamespace, operatorImage string, debug bool) error {
	if created, err := crds.Ensure(config); err != nil {
		return err
	} else if created {
		reporter.Succeeded("Created operator CRDs")
	}

	if created, err := namespace.Ensure(config, operatorNamespace); err != nil {
		return err
	} else if created {
		reporter.Succeeded(fmt.Sprintf("Created operator namespace: %s", operatorNamespace))
	}

	if created, err := serviceaccount.Ensure(config, operatorNamespace); err != nil {
		return err
	} else if created {
		reporter.Succeeded("Created operator service account and role")
	}

	if created, err := scc.Ensure(config, operatorNamespace); err != nil {
		return err
	} else if created {
		reporter.Succeeded("Updated the privileged SCC")
	}

	if created, err := lighthouseop.Ensure(reporter, config, operatorNamespace); err != nil {
		return err
	} else if created {
		reporter.Succeeded("Created Lighthouse service accounts and roles")
	}

	if created, err := deployment.Ensure(config, operatorNamespace, operatorImage, debug); err != nil {
		return err
	} else if created {
		reporter.Succeeded("Deployed the operator successfully")
	}
