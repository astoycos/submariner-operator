...
func Ensure(reporter eventreporter.Reporter, config *rest.Config, operatorNamespace string) (bool, error) {
	if created, err := serviceaccount.Ensure(config, operatorNamespace); err != nil {
		...
		reporter.Succeeded("Created lighthouse service account and role")
	}
